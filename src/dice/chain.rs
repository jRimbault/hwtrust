    ...
    pub(crate) fn validate(
        ...
        for (n, payload) in payloads.iter().enumerate() {
            ...
            if subjects.replace(payload.subject()).is_some() {
                return Err(ValidationError::RepeatedSubject(
                    n,
                    payload.subject().to_string(),
                ));
            }
            if keys
                .replace(payload.subject_public_key().to_pem())
                .is_some()
            {
...
mod tests {
    ...
    fn chain_validate_valid() {
        ...
        let payloads = keys
            .map(|(n, key)| valid_payload(n, key).build().unwrap())
            .collect();
    ...
    fn chain_validate_fails_with_mismatching_issuer_and_subject() {
        ...
        assert_eq!(
            err,
            ValidationError::IssuerMismatch(1, wrong.into(), expected.into())
        );
    }
