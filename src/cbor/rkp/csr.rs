...
use crate::cbor::{self, value_from_bytes};
...
#[derive(Debug, thiserror::Error)]
enum Constraint {
    #[error("CSR CBOR is an empty array")]
    EmptyArray,
    #[error("expected array, found: {0:?}")]
    ExpectedArray(ciborium::Value),
    #[error("Expected a map for verified device info, found '{0:?}'")]
    ExpectedMap(Option<ciborium::Value>),
    #[error("Expected integer or array at index {VERSION_OR_DEVICE_INFO_INDEX}, found {0:?}")]
    ExpectedIntOrArray(ciborium::Value),
    #[error("Device info should contain exactly {0} entries")]
    FixedLength(usize),
}

impl Csr {
    /// Parse base64-encoded CBOR data as a Certificate Signing Request.
    pub fn from_base64_cbor<S: AsRef<[u8]>>(session: &Session, base64: &S) -> Result<Self, Error> {
        let cbor: Vec<u8> = BASE64_STANDARD.decode(base64).or(Err(Error::InvalidCsr))?;
        Self::from_cbor(session, cbor.as_slice())
    }

    /// Read and parse CBOR data as a Certificate Signing Request.
    pub fn from_cbor(session: &Session, cbor: &[u8]) -> Result<Self, Error> {
        let value: Value = ciborium::de::from_reader(cbor).or(Err(Error::InvalidCbor))?;
        let mut array = match value {
            Value::Array(a) if a.is_empty() => Err(Constraint::EmptyArray)?,
            Value::Array(a) => a,
            other => Err(Constraint::ExpectedArray(other))?,
        };
        ...
        match version_or_device_info {
            ...
            other => Err(Constraint::ExpectedIntOrArray(other))?,
        }
    }
    ...
    fn v3_from_authenticated_request(
        ...
        if version != 1 {
            bail!(
                "Invalid CSR version. Only '1' is supported, found '{}",
                version
            );
        }
        ...
        let signed_data_payload = signed_data
            .payload
            .context("missing payload in SignedData")?;
        ...
        } = csr
        {
            ...
            match dice_chain {
                ChainForm::Proper(proper_chain) => {
                    ...
                    assert_eq!(
                        payloads[0].config_desc().component_name(),
                        Some("component_name")
                    );
